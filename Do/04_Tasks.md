...
- **T003** PostgreSQLローカルインストールと起動確認
  - モジュール：基盤整備 / インフラ
  - 優先度：**クリティカル**
  - 推定工数：中（2-3時間）
  - 依存関係：T001
  - 内容：PostgreSQL 15をローカルにインストール、データベース作成、接続確認

- **T004** Python環境構築と依存パッケージインストール
  - モジュール：基盤整備
  - 優先度：**高**
  - 推定工数：小（1-2時間）
  - 依存関係：T001
  - 内容：Python 3.11+ venv作成、yfinance、pandas等のインストール
...
- **T013** PostgreSQL初期化と接続確認
  - モジュール：データベース基盤
  - 優先度：**クリティカル**
  - 推定工数：小（1-2時間）
  - 依存関係：T003
  - 内容：PostgreSQL接続確認、初期ユーザー・DB作成
...
- [ ] フェーズ1完了時：ローカル環境で全サービスが起動、DBスキーマ確認
